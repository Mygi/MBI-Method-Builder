 ...
 * The definitive nifti-1 specification is the nifti1.h file on the <a href="http://nifti.nimh.nih.gov/dfwg/">NIfTI DFWG
 * website </a>
 * 
 * <p>
 * Notes: <br>
 ...
 * 
 * <p>
 * Endian-ness: Java uses big-endian by default. nifti files determined to be little-endian as per the nifti spec
 * (dim[0] field) will have the header and data swapped on read. (Not sure about write yet...)
 * 
 * <p>
 * Datatypes: the nifti-1 format supports a large number of datatypes. The current read methods typecast all data to
 * doubles. This API will be extended to include methods to return image blobs of different datatypes.
 * 
 * 
 * <h3>Todo List</h3>
 * <ul>
 * 
 * <li>
 * This API currently uses the traditional Java java.io methods. Will explore performance gains by using the java.nio
 * methods, including possibly memory mapping. I think some of the new java.io methods such as readFully() use nio
 * anyway.</li>
 * 
 * <li>
 * For header read and write, should change to read/write fields from/to 348 byte blob, and read/write blob from disk in
 * 1 call.</li>
 * 
 * <li>
 * Currently the writeHeader() method writes from the lower level variables, not the "shortcut" variables ie pixdim[0]
 * not qfac, dim[1] not XDIM, etc. need to think about this, use accessor methods</li>
 * 
 * <li>
 * Will need to change read/writeVol methods to call read/write slice methds b/c blob_size has to be an int but should
 * be a long. 32 bit int only allows max volume 1625^3 which is too small.</li>
 * 
 * <li>
 * Add methods to manipulate qform/sform and roatation matrices and quaternion representations</li>
 * 
 * <li>
 * Add simple method to make a Java AWT image from a slice</li>
 * 
 * <li>
 * <br>
 * Possible extensions to this API: Extend this API to be compatible with Analyze7.5 format ?</li>
 * </ul>
 * 
 * 
 * 
...
public class NiftiDataSet {
	...
	//
	// ////////////////////////////////////////////////////////////////
	public static final String NIFTI_PAIR_HDR_EXT = ".hdr";
	public static final String NIFTI_PAIR_IMG_EXT = ".img";
	public static final String NIFTI_EXT = ".nii";
	public static final String GZIP_EXT = ".gz";
	public static final int NIFTI_PAIR_HDR_SIZE = 348;
	public static final long NIFTI_HDR_SIZE = 352;
	public static final int EXT_KEY_SIZE = 8; // esize+ecode
	public static final String NIFTI_MAGIC_STRING = "n+1";
	public static final String NIFTI_PAIR_MAGIC_STRING = "ni1";
	...
	public boolean _bigEndian; // does hdr appear to have BE format
	...
	public byte[] data_type_string = new byte[10]; // 10 char UNUSED
	 ...
	 * 
	 * @param name
	 *            - name of the nifti-1 dataset. The name can have the .hdr, .img, or .nii extension, optionally
	 *            followed by a .gz compression suffix. Or, the name can be specified with no extensions or suffixes, in
	 *            which case the program will look in this order for files with these extensions: .hdr<.gz> .img<.gz>
	 *            .nii<.gz>
	 */
	public NiftiDataSet(String name) {
	...
	// ////////////////////////////////////////////////////////////////
	/**
	 * Constructor for creation of a new dataset. Default values are set, programmer must set or reset all fields
	 * appropriate for the new dataset.
	 * 
	 */
	public NiftiDataSet() {
	...
	public void readHeader() throws IOException, FileNotFoundException {
		...
		try {
			...
			if (ds_hdrname.endsWith(".gz"))
				ecs = new EndianCorrectInputStream(new GZIPInputStream(new FileInputStream(ds_hdrname)), _bigEndian);
			else
				ecs = new EndianCorrectInputStream(ds_hdrname, _bigEndian);

			sizeof_hdr = ecs.readIntCorrect();


			ecs.readFully(data_type_string);
	...
	//
	// //////////////////////////////////////////////////////////////////
	public void copyHeader(NiftiDataSet A) {
		...
		_bigEndian = A._bigEndian;
		sizeof_hdr = A.sizeof_hdr;
		data_type_string = new byte[10];
	 ...
	 * 
	 * @param ecs
	 *            is an InputStream open and pointing to begining of extensions array
	 * @exception IOException
	 */
	private void readNiiExt(EndianCorrectInputStream ecs) throws IOException {
		...
		if (extension[0] != (byte) 0) {
			start_addr = NIFTI_PAIR_HDR_SIZE + 4;
	 ...
	 * 
	 * @param ecs is an InputStream open and pointing to begining of extensions array
	 * 
	 * @exception IOException
	 */
	 ...
	 * 
	 * @return nx2 array where n = # of extensions, array elem 0 is the size in bytes of that extension and array elem 1
	 ...
	 * 
	 * @param index
	 *            number of the extension to remove (0 based)
	 */
	 ...
	 * 
	 * @param code
	 *            -- code identifying the extension
	 * @param filename
	 *            -- filename containing the extension. The entire file will be added as an extension
	 */
	...
	public void writeHeader() throws IOException, FileNotFoundException {
		...
		try {
			...
			ecs = new EndianCorrectOutputStream(baos, _bigEndian);

			ecs.writeIntCorrect(sizeof_hdr);

			ecs.write(data_type_string);
		...
		try {
			...
			throw new IOException("Error: unable to write header extensions for file " + ds_hdrname + ": "
					+ ex.getMessage());
		}
	...
	public void printHeader() {
		...
		System.out.println("Slice timing code:\t\t\t\t" + slice_code + " (" + decodeSliceOrder((short) slice_code)
				+ ")");
		System.out.println("MRI slice ordering (freq, phase, slice index):\t" + freq_dim + " " + phase_dim + " "
				+ slice_dim);
	 ...
	 * 
	 * @param d
	 *            1D double array of timecourse values, length TDIM
	 */
	...
	// ////////////////////////////////////////////////////////////////
	/**
	 * Check if a valid dataset (header+data) exists. Note that some operations (e.g. header editing) do not actually
	 * require that a data file exist. Use existsHdr() existsDat() in those cases. Gzipped files with .gz extension are
	 * permitted.
	 * 
	...
	public void setHeaderFilename(String s) {
		...
		ds_hdrname = s;
		if (ds_is_nii) {
			if (!ds_hdrname.endsWith(NIFTI_EXT))
				ds_hdrname = ds_hdrname + NIFTI_EXT;
		} else {
			if (!ds_hdrname.endsWith(NIFTI_PAIR_HDR_EXT))
				ds_hdrname = ds_hdrname + NIFTI_PAIR_HDR_EXT;
		}
		return;
	}
	...
	public void setDataFilename(String s) {
		ds_datname = s;
		if (ds_is_nii) {
			if (!ds_datname.endsWith(NIFTI_EXT))
				ds_datname = ds_datname + NIFTI_EXT;
		} else {
			if (!ds_datname.endsWith(NIFTI_PAIR_IMG_EXT))
				ds_datname = ds_datname + NIFTI_PAIR_IMG_EXT;
		}
	...
	// ////////////////////////////////////////////////////////////////
	/*
	 * Set fields to make this a nii (n+1) (single file) dataset switching from nii to anz/hdr affects -- magic field
	 * "n+1\0" not "ni1\0" -- vox_offset must include 352+extensions -- internal ds_is_nii flag
	 * 
	 * NOTE: all changes are in memory, app still needs to write header and data for change to occur on disk
	 * 
	...
	private void setToNii() {
		...
		ds_is_nii = true;
		magic = new StringBuffer(NIFTI_MAGIC_STRING);

		vox_offset = NIFTI_HDR_SIZE;
		if (extension[0] != 0) {
			extlist = getExtensionsList();
			n = extlist.length;
			for (i = 0; i < n; i++)
	...
	// ////////////////////////////////////////////////////////////////
	/*
	 * Set fields to make this a ni1 (2 file img/hdr) dataset switching from nii to anz/hdr affects -- magic field
	 * "n+1\0" vs "ni1\0" -- vox_offset does notinclude 352+extensions -- internal ds_is_nii flag
	 * 
	 * NOTE: all changes are in memory, app still needs to write header and data for change to occur on disk
	 * 
	 * maybe add auto set of dat name to hdr name, strip img/hdr ??
	...
	private void setToNi1() {
		...
		ds_is_nii = false;
		magic = new StringBuffer(NIFTI_PAIR_MAGIC_STRING);
	 ...
	 * 
	 * @return datatype code (note: it is not guaranteed to be a valid code, what is there is what you get...)
	 */
	...
	private void checkName(String name) {
		...
		if (!wname.endsWith(NIFTI_PAIR_HDR_EXT) && !wname.endsWith(NIFTI_PAIR_IMG_EXT) && !wname.endsWith(NIFTI_EXT)) {
			// files to look for
			nii_file = new File(wname + NIFTI_EXT);
			niig_file = new File(wname + NIFTI_EXT + GZIP_EXT);
			anzh_file = new File(wname + NIFTI_PAIR_HDR_EXT);
			anzhg_file = new File(wname + NIFTI_PAIR_HDR_EXT + GZIP_EXT);
			anzd_file = new File(wname + NIFTI_PAIR_IMG_EXT);
			anzdg_file = new File(wname + NIFTI_PAIR_IMG_EXT + GZIP_EXT);

			if (nii_file.exists())
				wname = wname + NIFTI_EXT;
			else if (niig_file.exists())
				wname = wname + NIFTI_EXT;
			else if (anzh_file.exists())
				wname = wname + NIFTI_PAIR_HDR_EXT;
		...
		if (wname.endsWith(NIFTI_PAIR_HDR_EXT)) {
			ds_hdrname = new String(wname);
			ds_datname = wname.substring(0, wname.length() - NIFTI_PAIR_HDR_EXT.length()) + NIFTI_PAIR_IMG_EXT;
		}

		else if (wname.endsWith(NIFTI_PAIR_IMG_EXT)) {
			ds_datname = new String(wname);
			ds_hdrname = wname.substring(0, wname.length() - NIFTI_PAIR_IMG_EXT.length()) + NIFTI_PAIR_HDR_EXT;
		}

		else if (wname.endsWith(NIFTI_EXT)) {
			ds_datname = new String(wname);
			ds_hdrname = new String(wname);
			ds_is_nii = true;
		}
	...
	private void setDefaults() {
		...
		if (bo == ByteOrder.BIG_ENDIAN)
			_bigEndian = true; // endian flag, need to flip little end.
		else
			_bigEndian = false;
		sizeof_hdr = NIFTI_PAIR_HDR_SIZE; // must be 348 bytes
		data_type_string = new byte[10]; // 10 char UNUSED
		Arrays.fill(data_type_string, (byte)0);
		...
		magic = new StringBuffer(NIFTI_MAGIC_STRING); // 4 char id must be "ni1\0" or "n+1\0"
	 ...
	 * @return 3D double array, scale and offset have been applied. Array indices are [Z][Y][X], assuming an xyzt
	 *         ordering of dimensions. ie indices are data[dim[3]][dim[2]][dim[1]]
	 * @exception IOException
	 * 
	 */
	public double[][][] readDoubleVol(short ttt) throws IOException {
		...
		ecs = new EndianCorrectInputStream(new ByteArrayInputStream(b), _bigEndian);
		switch (datatype) {
	...
	// ////////////////////////////////////////////////////////////////
	/**
	 * Write one 3D double array to disk. Data is written in datatype of this Nifti1Dataset instance. Data is
	 * "un-scaled" as per scale/offset fields of this Nifti1Dataset instance, before being written to disk.
	 * 
	 * @param data
	 *            3D array of array data Array indices are [Z][Y][X], assuming an xyzt ordering of dimensions. ie
	 *            indices are data[dim[3]][dim[2]][dim[1]]
	 * @param ttt
	...
	// ////////////////////////////////////////////////////////////////
	/*
	 * Read one 3D volume blob from disk to byte array ttt T dimension of vol to read (0 based index) return byte array
	 * 
	 * @exception IOException
	 */
	...
	// ////////////////////////////////////////////////////////////////
	/*
	 * Write one 3D volume blob from byte array to disk ByteArrayOutputStream baos -- blob of bytes to write ttt T
	 * dimension of vol to write (0 based index)
	 * 
	 * @exception IOException
	 */
	...
	// ////////////////////////////////////////////////////////////////
	/**
	 * Read all the data into one byte array. Note that since the data is handled as bytes, NO byte swapping has been
	 * performed. Applications converting from the byte array to int/float datatypes will need to swap bytes if
	 * necessary. return byte array with all image data
	 * 
	 * @exception IOException
	 */
	...
	// ////////////////////////////////////////////////////////////////
	/**
	 * Write a byte array of data to disk, starting at vox_offset, beginning of image data. It is assumed that usually
	 * the entire data array will be written with this call. Note that since the data is handled as bytes, NO byte
	 * swapping will be performed. Applications needing byteswapping must swap the bytes correctly in the input array b.
	 * 
	 * @param b
	 ...
	 * 
	 * @param x
	 *            X dimension of vol to read (0 based index)
	 * @param y
	 *            Y dimension of vol to read (0 based index)
	 * @param z
	...
	public double[] readDoubleTmcrs(short x, short y, short z) throws IOException {
		...
		// get input handle
		if (ds_datname.endsWith(".gz"))
			ecs = new EndianCorrectInputStream(new GZIPInputStream(new FileInputStream(ds_datname)), _bigEndian);
		else
			ecs = new EndianCorrectInputStream(new FileInputStream(ds_datname), _bigEndian);
	 ...
	 * 
	 * @return s String of length n with as much of s as possible, padded with 0 if necessary
	 */
	...
	public static void main(String[] args) {
		...
		NiftiDataSet nds = new NiftiDataSet(args[0]);
		try {
			nds.readHeader();
			nds.printHeader();
		} catch (IOException ex) {
