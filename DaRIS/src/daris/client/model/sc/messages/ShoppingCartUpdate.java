...
public class ShoppingCartUpdate extends ObjectMessage<Boolean> {
	...
	public ShoppingCartUpdate(long scid, String name,
			DeliveryDestination destination, Archive archive,
	...
	public ShoppingCartUpdate(long scid) {
	...
	public ShoppingCartUpdate(ShoppingCart cart) {
		...
		this(cart.scid(), cart.name(), cart.destination(), cart.archive(), cart
				.medatadataOutput(), cart.transcodes());
	}
		...
		if (_archive != null) {
			...
			if (_archive.hasParameters()) {
				for (Archive.Parameter parameter : _archive.parameters()) {
					w.add("parameter",
							new String[] { "name", parameter.name() },
