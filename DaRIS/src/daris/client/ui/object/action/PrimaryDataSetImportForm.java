...
public class PrimaryDataSetImportForm extends ValidatedInterfaceComponent
	...
	public PrimaryDataSetImportForm(List<LocalFile> files, DObjectRef o,
			PrimaryDataSetCreateFCP fcp) {
		_o = o;
		_fcp = fcp;
		_fcp.setPid(o.id());
		...
		Field<String> pidField = new Field<String>(new FieldDefinition(
				"Parent " + _o.referentTypeName(), ConstantType.DEFAULT,
				"Parent " + _o.referentTypeName(), null, 1, 1));
		pidField.setValue(_fcp.pid());
		form.add(pidField);
		...
		Field<String> nameField = new Field<String>(new FieldDefinition(
				...
				"Name", null, 1, 1));
		nameField.setValue(_fcp.dataSetName());
		nameField.addListener(new FormItemListener<String>(){

			@Override
			public void itemValueChanged(FormItem<String> f) {
				_fcp.setDataSetName(f.value());
			}
		...
		AssetImportTask.create((NamespaceRef) null, _files,
				...
				new DTITaskCreateHandler<AssetImportTask>() {
					...
					public void failed() {
						l.executed(false);
						Dialog.inform("Error",
								"Failed to create DTI task for imporing derivation dataset");
